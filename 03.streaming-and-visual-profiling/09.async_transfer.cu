...
int main() {
    ...
    const int segmentN = N / numOfSegments;
    const size_t segmentSize = size / numOfSegments;
    for (int i = 0; i < numOfSegments; i++) {
        cudaStream_t segStream;
        cudaStreamCreate(&segStream);

        int segmentOffset = i * segmentN;

        addVectorsInto<<<numberOfBlocks, threadsPerBlock, 0, segStream>>>(
            &c[segmentOffset], &a[segmentOffset], &b[segmentOffset], segmentN);

        cudaMemcpyAsync(&h_c[segmentOffset], &c[segmentOffset], segmentSize,
                        cudaMemcpyDeviceToHost, segStream);

        addVectorsErr = cudaGetLastError();
        if (addVectorsErr != cudaSuccess)
            printf("Error: %s\n", cudaGetErrorString(addVectorsErr));

        cudaStreamDestroy(segStream);
    }
